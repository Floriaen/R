...
function Monocle.new(initial)
	...
	Monocle.padding = initial.padding or 10
	...
	Monocle.printerPreffix = initial.printerPreffix or '[Monocle]: '
...
function Monocle.print(text,justtext)
	if Monocle.printer and not justtext then
		print(Monocle.printerPreffix .. text)
	elseif justtext then
		return Monocle.printerPreffix .. text
	end
end
	...
	if Monocle.active then
		...
		for name,result in pairs(Monocle.results) do
			if type(result) == 'number' or type(result) == 'string' then
				love.graphics.print(Monocle.names[name] .. " : " .. result, Monocle.padding, (draw_y + 1) * 15)
			elseif type(result) == 'table' then
				love.graphics.print(Monocle.names[name] .. " : Table:", Monocle.padding, (draw_y + 1) * 15)
				draw_y = draw_y + 1
				for i,v in pairs(result) do
					love.graphics.print("      " .. i .. " : " .. v, Monocle.padding, (draw_y + 1) * 15)
					draw_y = draw_y + 1
				end
			end
			draw_y = draw_y + 1
